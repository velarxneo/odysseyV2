...
module minter::mint_stage_tests {
    ...
    fun test_no_allowlist_execution(creator: &signer, user: &signer, aptos_framework: &signer) {
        ...
        // User has not minted yet, so their no allowlist user balance should be 0
        assert!(mint_stage::user_balance_in_no_allowlist(mint_stage_data, category, user_addr) == option::none(), 0);

        // User tries to mint 1 token, should be successful
        mint_stage::assert_active_and_execute(user, mint_stage_data, category, amount);
        // Verify user balance in no_allowlist is updated
        assert!(
            mint_stage::user_balance_in_no_allowlist(mint_stage_data, category, user_addr) == option::some(amount),
            0,
        );
    }
