...
export const uploadNFT = async (
  ...
  console.log(
    `Uploaded Image ${tokenID}.${GlobalImageType}: ${imageTransaction.id}`
  );
...
function checkFilesExist(
  ...
  for (let i = startIndex; i <= endIndex; i++) {
    if (
      !fs.existsSync(`${assetsFolderName}/${i}.${GlobalImageType}`) ||
      !fs.existsSync(`${assetsFolderName}/${i}.json`)
    ) {
  ...
  try {
    ...
    for (let i = 0; i < jsonFiles.length; i++) {
      ...
      const jsonData = JSON.parse(
        fs.readFileSync(`${assetsFolderName}/${jsonFilename}`, "utf-8")
      );
      ...
      if (
        ...
        console.log(
          `Error: Mandatory metadata field missing in ${jsonFilename}`
        );
        result = false;
      }
      ...
      for (const creator of jsonData.properties.creators) {
        if (!creator.address || !creator.share) {
          console.log(
            `Error: Missing 'address' or 'share' field in creators array in ${jsonFilename}`
          );
          result = false;
        }
      }
      ...
      if (!Array.isArray(jsonData.attributes)) {
        console.log(
          `Error: 'attributes' field should be an array in ${jsonFilename}`
        );
        result = false;
      }
      ...
      for (const attribute of jsonData.attributes) {
        if (!attribute.trait_type || !attribute.value) {
          console.log(
            `Error: Missing 'trait_type' or 'value' field in attributes array in ${jsonFilename}`
          );
          result = false;
        }
      }
